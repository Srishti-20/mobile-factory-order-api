...
5. Checking the validity of the input:
    ...
    components.sort()
    ```
- the first condition is to check if the `components` is a list datatype or not.
- The second condition is to check if `components` has values in it or if is it an empty list, and to verify its datatype too.
- then we sort the `components`, to process the output in the right order of categories for the required data.
...
6. Data processing and formatting
    ...
    # Check if the component code is valid
...
if component_info is not None, variable `category` will store the value of the key called category taken from the component_info. Then it is checked that if the catagory is already present in the selected_categories set() or not, if yes then shows an error, if not then add that category to the set().
...
8. Generate and return the order_details/output
    ...
    order_response = {
        '1order_id': generate_order_id(),                       # Generate a unique order ID
        '2total': total_price,                                  # Total price of the order
        '3parts': order_parts                                   # List of parts included in the order
    }
